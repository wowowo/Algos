...
import girls_got_rhytms.Edge;
import girls_got_rhytms.WeightedEdgeGraph;
...
public class FloydWarshall {
	...
	public static void allPairsPath(WeightedEdgeGraph graph) {
		...
		for(int k = 1; k < V; k++)
			for(int i = 1; i < V; i++)
				for(int j = 1; j < V; j++)
					check[i][j][k] = Math.min(check[i][j][k-1], check[i][k][k-1] + check[k][j][k-1]);
